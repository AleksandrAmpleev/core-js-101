...
function getFactorial(n) {
  if (typeof (n) === 'number' && n < 13 && n > -1) { return (n > 1) ? n * getFactorial(n - 1) : 1; }

  throw new RangeError(`${n.toString()}=n`);
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let ret = '';
  if (isStartIncluded) { ret += '['; } else { ret += '('; }
  if (a < b) {
    ret += `${a}, ${b}`;
  } else {
    ret += `${b}, ${a}`;
  }
  if (isEndIncluded) { ret += ']'; } else { ret += ')'; }
  return ret;
}
...
function isBracketsBalanced(/* str */) {
  ...
  // return (sk1.length === sk2.length) && (sk3.length === sk4.length)
  //  && (sk5.length === sk6.length) && (sk7.length === sk8.length);
  throw new Error('Not implemented');
}
...
function toNaryString(num, n) {
  const ConvertNumber = (number) => (from) => (to) => parseInt(number, from).toString(to);
  const f = (number) => ConvertNumber(number)(10)(n);
  const t = f(num);
  return t.toString();
}
...
function getCommonDirectoryPath(pathes) {
  ...
  pathes.map((item) => {
    zeroPath.push(item.split('/'));
    return 1;
  });

  zeroPath[0].map((item, index) => {
    let contains = true;

    zeroPath.map((itemp) => {
      if (((itemp.length > index) && (itemp[index] !== item)) || (itemp.length <= index)) {
        contains = false;
      }
      return 1;
    });
